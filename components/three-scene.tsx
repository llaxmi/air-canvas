...
function downsample(points: { x: number; y: number }[], targetCount: number): { x: number; y: number }[] {
...
const ClayTube: React.FC<ClayTubeProps> = memo(({ stroke, index, canvasWidth, canvasHeight }) => {
  ...
  const geometry = useMemo(() => {
    ...
    const downsampled = downsample(normalizedPoints, Math.min(normalizedPoints.length, 30));
...
// Invalidate on demand for better performance
...
const ThreeScene: React.FC<ThreeSceneProps> = memo(({ strokes, canvasWidth, canvasHeight }) => {
  return (
